...
- I3. Never install into `~` or system paths — only into a project folder under version control, or a folder the CEO explicitly named. **Recorded exception (GATE-028, 2026-07-10):** the distribution kit's installer MAY write exactly two files under `~/.claude` — a copy of this skill at `~/.claude/skills/qroky/SKILL.md` and a marker-guarded trigger block in `~/.claude/CLAUDE.md` — strictly on the owner's explicit opt-in at install (interview question 9, a typed yes; Enter declines), idempotent and removable (both paths are named to the owner at install and in the kit README). Nothing else under `~`, ever; this exception extends to no other installer and no other path.
